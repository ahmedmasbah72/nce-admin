...
class AdminCartController extends Controller
{
    public function addToCart(Request $request, $id)
    {
        ...
        return redirect()->route('admin.view')->with('success', 'Produit ajouté au panier!');
    }
    ...
    public function showCart()
    {
        ...
        return view('admin.admincart', compact('cart'));
    }
    ...
    public function updateCart(Request $request)
    {
        if ($request->id && $request->quantity && $request->price) {
            ...
            $cart[$request->id]["discount"] = $request->discount ?? 0;
            session()->put('cart', $cart);
            session()->flash('success', 'Panier mis à jour avec succès');
        }
        return redirect()->route('admin.admincart');
    }
